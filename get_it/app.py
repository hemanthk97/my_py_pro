...
class nodevisit(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        global dict1
        dict1.update({str(node.lineno):'Function_def'})
        self.generic_visit(node)
    def visit_If(self, node):
        global dict1
        dict1.update({str(node.lineno):'If'})
        self.generic_visit(node)
    def visit_While(self, node):
        global dict1
        dict1.update({str(node.lineno):'While'})
        self.generic_visit(node)
    def visit_Assign(self, node):
        global dict1
        #print(ast.dump(node))
        if isinstance(node.value, ast.Num):
            dict1.update({str(node.lineno):('Assign','Num',node.value.n,node.targets[0].id)})
        elif isinstance(node.value, ast.Str):
            dict1.update({str(node.lineno):('Assign','Str',node.value.s,node.targets[0].id)})
        elif isinstance(node.value, ast.NameConstant):
            dict1.update({str(node.lineno):('Assign','Str',node.value.value,node.targets[0].id)})
        else:
            ...
            dict1.update({str(node.lineno):('Assign','Expression',node.targets[0].id,var_names,func_names)})
        self.generic_visit(node)
    def visit_Expr(self, node):
        global dict1
        #print(ast.dump(node))
        dict1.update({str(node.lineno):'Expr'})
        self.generic_visit(node)
    def visit_Return(self,node):
        global dict1
        dict1.update({str(node.lineno):'Return'})
        self.generic_visit(node)
...
def compileFunction(e,code):
    ...
    if excepName == "WrongNumArguError":
        res = {"error_name":"NumofArugments","error_message":str(data["NumofArugments"])+" '"+str(e.args[1])+"'","error":"1","error_Type":"compile","line_no":0}
        return jsonify(res)
    ...
    try:
       ...
       if ans != 1:
        ...
        if excepName == "NameError":
           ...
           res = {"resData":l_execution_path,"error_name":excepName,"error_message":"'"+name+"' "+data['NameError'],"error":"1","error_Type":"run-time","line_no":line_number,'Details_with_linenum':dict1}
           i = 0
           dict1 = {}
           return jsonify(res)
        res = {"resData":l_execution_path,"error_name":excepName,"error_message":str(e),"error":"1","error_Type":"run-time","line_no":line_number,'Details_with_linenum':dict1}
        i = 0
        dict1 = {}
        return jsonify(res)
    res = {"resData":l_execution_path,"error":"0",'Details_with_linenum':dict1}
    i = 0
    dict1 = {}
    return jsonify(res)
